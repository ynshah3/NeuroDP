    ...
    for run in range(runs):
        ...
        for value in np.array(values.split(',')).astype(float):
            ...
            for fold, (train_idx, val_idx) in enumerate(kfold.split(ten_folds_dataset, ten_folds_dataset.targets)):
                ...
                fpath_lesion_stats = dpath + 'run_' + str(run) + '_' + str(fold) + '_lesion_stats.txt'
                ...
                metrics = exp.run_experiment(train_loader, val_loader)
                ...
                log_to_file(fpath_lesion_stats, ','.join(str(x) for x in metrics['lesion_stats']))
